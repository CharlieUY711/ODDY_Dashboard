...
import { CreditCard, CheckCircle, AlertCircle, Settings, Eye, EyeOff, Zap, Loader2 } from 'lucide-react';
...
export function MetodosPagoView({ onNavigate }: Props) {
  ...
  const loadMetodos = async () => {
    ...
    setError(null);
    try {
      const res = await api.checkout.getMetodosPago();
      if (res.ok && res.data) {
        setMetodos(res.data as MetodoPago[]);
      } else {
  ...
  const toggleActivo = async (id: string, nuevoEstado: boolean) => {
    ...
    try {
      ...
      // En producción, necesitarías agregar api.checkout.updateMetodoPago(id, { activo: nuevoEstado })
      setMetodos(prev => prev.map(m => m.id === id ? { ...m, activo: nuevoEstado } : m));
      // Aquí iría la llamada real a la API cuando esté disponible
      // const res = await api.checkout.updateMetodoPago(id, { activo: nuevoEstado });
    } catch (err) {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 22 }}>
          {[
            ...
            { label: 'Última actualización', value: 'Ahora', color: '#8B5CF6' },
          ].map((k, i) => (
        ...
        {loading ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#9CA3AF' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.9rem' }}>Cargando métodos de pago...</p>
          </div>
        ) : metodos.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#9CA3AF' }}>
            <CreditCard size={32} color="#E5E7EB" style={{ marginBottom: 12 }} />
            <p style={{ margin: 0, fontSize: '0.9rem' }}>No hay métodos de pago configurados</p>
          </div>
        ) : (
          /* Lista de métodos */
              ...
              <div key={m.id} style={{ backgroundColor: '#fff', borderRadius: '14px', border: `1px solid ${m.activo ? '#E5E7EB' : '#F3F4F6'}`, padding: '20px', opacity: m.activo ? 1 : 0.7 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: '#9CA3AF' }}>Tipo: {m.tipo}</div>
                  </div>
